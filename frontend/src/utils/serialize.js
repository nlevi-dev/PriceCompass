...
export async function deserializeBin(arrayBuffer, lang="EN") {
    ...
    for (let i = 0; i < combinationCount; i++) {
        const itemIdx    = bitsToIntDV(dv, pos, BITS_ITEM_LEN);
        pos += BITS_ITEM_LEN;
        const countryIdx = bitsToIntDV(dv, pos, BITS_COUNTRY_LEN);
        pos += BITS_COUNTRY_LEN;
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const link          = parts[1];
        let original_name = parts.slice(2, parts.length).join(",");
        if (original_name[0] === '"' && original_name[original_name.length - 1] === '"')
            original_name = original_name.slice(1, -1);
