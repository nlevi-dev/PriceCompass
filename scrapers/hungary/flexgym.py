...
def parse_item(node, categories, url, place):
    ...
    cols = [c for c in rows[0].children if isinstance(c, Tag)]
    adult_idx = None
...
def get_items_base(url, categories, place, use_cache = True, cache_time = None):
    ...
    nodes = soup.find_all(name="table", attrs={"class":"has-fixed-layout"})
    items = []
    for node in nodes:
...
def get_items(use_cache = True, cache_time = None):
    urls = [
        ("https://www.flexgym.hu/arak/",[Item.GYM_SINGLE_ENTRANCE,Item.GYM_MONTHLY_MEMBERSHIP,Item.GYM_YEARLY_MEMBERSHIP],"flexgym "),
    ]
    dfs = []
